...
parser.add_argument('--step', action='store',type=str,
			help="step3 or step4")
...
oldlink='https://jiwoong.web.cern.ch/jiwoong/cgi-bin/igprof-navigator/'+ args.old + step + level
newlink='https://jiwoong.web.cern.ch/jiwoong/cgi-bin/igprof-navigator/'+ args.new + step + level
...
for Tot_str in Tot_strs:
    ...
    print("[{0:<4} -> {1:<4}] [{2:<6} -> {3:<6}]  [{4:<9.4E} -> {5:<9.4E}]         [ {6:<9.4E}  - {7:<9.4E} / {8:<9.4E} *100% ] = [{9:<6}% ]         {10:<50}"\
          .format(idx_old[0],idx_new[0],global_old_df.loc[idx_old[0]]['total'],global_new_df.loc[idx_new[0]]['total']\
